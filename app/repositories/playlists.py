...
def get_playlist(db: Session, playlist_id: int) -> UserPlaylist | None:
    stmt = playlist_options(select(UserPlaylist).where(UserPlaylist.id == playlist_id))
...
def create_playlist(
    ...
    *,
    ...
    db.commit()
    db.refresh(playlist)
    return get_playlist(db, playlist.id) or playlist


def add_track_to_playlist(db: Session, *, playlist_id: int, track_id: int) -> UserPlaylist | None:
...
def remove_track_from_playlist(db: Session, *, playlist_id: int, track_id: int) -> UserPlaylist | None:
    playlist = get_playlist(db, playlist_id)
    if not playlist:
        return None
    existing = db.get(UserPlaylistTrack, {"playlist_id": playlist_id, "track_id": track_id})
